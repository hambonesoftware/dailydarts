...
export function createActionManager(
  ...
  const activeDarts = [];
  const particleScale =
    typeof options.particleScale === "number" ? options.particleScale : 1;
  ...
  return {
    ...
    needsRender: () => {
      return (
        ...
        (logo && typeof logo.isAnimating === "function" && logo.isAnimating()) ||
        (textPopups &&
          typeof textPopups.hasActivePopups === "function" &&
          textPopups.hasActivePopups()) ||
        (cameraRig && typeof cameraRig.isAnimating === "function" && cameraRig.isAnimating())
      );
    },
      ...
      for (let i = activeDarts.length - 1; i >= 0; i--) {
        ...
        if (dart.userData.isFlying) {
          ...
          if (easedT >= 1) {
            ...
            // Fireworks + confetti
            const fireworksCount = Math.max(60, Math.round(180 * particleScale));
            const confettiCount = Math.max(90, Math.round(260 * particleScale));
            ...
            if (confetti && typeof confetti.burst === "function") {
