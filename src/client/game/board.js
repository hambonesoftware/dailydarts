...
export function createDartboard(options = {}) {
  ...
  const numbersDelayMs =
    typeof options.numbersDelayMs === "number" ? options.numbersDelayMs : 0;
  ...
  if (showNumbers) {
    const loadNumbers = async () => {
      const [{ TextGeometry }, { FontLoader }] = await Promise.all([
    ...
    if (typeof window !== "undefined" && "requestIdleCallback" in window) {
      window.requestIdleCallback(scheduleNumbersLoad);
    } else {
      scheduleNumbersLoad();
    }
  }
