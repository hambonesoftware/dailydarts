...
export function createArcadeTextSystem(scene, opts = {}) {
  ...
  let gameFont = null;
  let textGeometryClass = null;
  let fontPromise = null;

  function ensureFont() {
    if (gameFont && textGeometryClass) {
      return Promise.resolve({ font: gameFont, TextGeometry: textGeometryClass });
    }
    ...
    if (!fontPromise) {
      ...
      ]).then(([geomModule, loaderModule]) => {
        textGeometryClass = geomModule.TextGeometry;
        const loader = new loaderModule.FontLoader();
        return new Promise((resolve) => {
          loader.load("/fonts/helvetiker_bold.typeface.json", (f) => {
            gameFont = f;
            resolve({ font: gameFont, TextGeometry: textGeometryClass });
          });
        });
      });
    }
  ...
  function spawnArcadeText(position) {
    if (!gameFont || !textGeometryClass) {
      void ensureFont().then(() => {
        spawnArcadeText(position);
      });
      return;
    }

    const makeLayer = (color, offsetZ, bevelSize) => {
      const geom = new textGeometryClass("DAILY DARTS", {
        font: gameFont,
        size: 0.8,
        height: 0.25,
        curveSegments: 14,
  ...
  return {
    spawnArcadeText,
    update,
    hasActivePopups: () => activePopups.length > 0,
    dispose,
  };
}
